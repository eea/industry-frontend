    ...
    >
      ...
      <TextArea
        ...
        onChange={({ target }) => {
          const template = {};
          try {
            template.value = JSON.parse(target.value) || {};
            onChange('blocks', null);
            onChange('blocks_layout', null);
            setTimeout(() => {
              if (template.value.blocks) {
                onChange('blocks', template.value.blocks);
              }
            }, 50);
            setTimeout(() => {
              if (template.value.blocks_layout) {
                onChange('blocks_layout', template.value.blocks_layout);
              }
            }, 100);
          } catch (e) {
            template.value = {};
