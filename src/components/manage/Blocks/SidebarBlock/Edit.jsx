...
import { SIDEBAR } from '~/constants/Blocks.js';
...
import {
  ...
  isEqual,
} from '@eeacms/volto-tabs-block/Tabs/utils';
...
class EditSidebarBlock extends React.Component {
  ...
  isFirstTabsBlock() {
    ...
    const [id] = blocks.find(([, value]) => value['@type'] === SIDEBAR);
  ...
  componentDidMount() {
    ...
    const res = blocks.find(([, value]) => value['@type'] === SIDEBAR);
  ...
  componentWillUnmount() {
    const blocks = getBlocks(this.context.contextData?.formData || {});
    const index = blocks.findIndex(([, value]) => value['@type'] === SIDEBAR);
    if (index === -1) {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      if (
        ...
        JSON.stringify(this.props.data.tabs)
      ) {
        schema = { ...getSchema(this.props) };
      }
      this.setState({ blocksLayout: new_layout, schema });
      setContextData({
        ...contextData,
        formData: {
          ...formData,
